...
- **Chat scroll behavior is owned by `mobile-template/src/chat/useChatScroll.ts`** (canonical) and copied verbatim into `layla-app/src/chat/useChatScroll.ts`. Contract: viewport stays where the user is looking unless they're at-bottom (sticky-follow); the "↓ Latest" pill surfaces only when the user has actively scrolled more than one full viewport above the bottom — never on programmatic content growth. **Do not add ad-hoc `scrollToEnd` / `scrollToIndex` calls in product ChatScreens.** Edit the hook + sync both copies.
