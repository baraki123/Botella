...
- **Chat scroll AND keyboard behavior is owned by `mobile-template/src/chat/useChatScroll.ts`** (canonical) and copied verbatim into `layla-app/src/chat/useChatScroll.ts`. The hook's top-of-file contract block is the single source of truth: sticky-bottom on at-bottom, never yank away from active reading, "↓ Latest" pill only on user-driven scroll-up of >1 viewport, re-snap on FlatList shrink (keyboard rise / sticky chip row), re-snap on `Keyboard.didShow / didHide`. The required `KeyboardAvoidingView` props (behavior + `KEYBOARD_VERTICAL_OFFSET_IOS`) are exported from the hook so screens import them — never re-decide. **Do not add ad-hoc `scrollToEnd` / `scrollToIndex` / `Keyboard.addListener` calls in product ChatScreens, Bubble, or Composer.** Edit the hook + sync both copies.
