      ...
      >
        ...
        <View style={styles.listWrap}>
          <FlatList
            ...
            // reach them. The user crossed ~50 total messages after just 2-3
            // free-chat turns (onboarding ~30 + get-to-know ~12 already sits
            // at ~45), so Layla's reply landed in state + the persisted tail
            // but never mounted in the DOM — looked exactly like "Layla
            // stopped responding." (Bumping the constant just moves the
            // cliff.) Driving `initialNumToRender` from the data length keeps
            // the tail mounted at every render, removeClippedSubviews keeps
